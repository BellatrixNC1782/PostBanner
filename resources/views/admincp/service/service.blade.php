        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <div class="page-leftheader">
                ...
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><a>Service Management</a></li>
                </ol>
            </div>
        </div>
            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="text-end">
                            <a class="" href="{{route('addservice')}}">
                                <button type="button" class="btn btn-primary">Add Service</button>
                            </a>
                        </div>
                    </div>
                    <div class="card-body">
                        <table id="serviceDatatable" class="table table-bordered text-nowrap w-100">
                            <thead>
                                <tr>
                                    <th>Sr No.</th>
                                    <th>Service Name</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        table = jQuery('#serviceDatatable').DataTable({
            "processing": true,
            "serverSide": true,
            "responsive": true,
            "order": [
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        ...
                        var id = row.id;
                        if (status == 'active') {
                            return '<button title="Active" class="btn btn-success waves-effect waves-light" onClick="faqStatusChange(' + id + ')">Active</button>';
                        } else {
                            return '<button title="Deactive" class="btn btn-danger waves-effect waves-light" onClick="faqStatusChange(' + id + ')">Deactive</button>';
                        }
                    }
                },
                {
                    ...
                    "render": function(data, type, row) {
                        ...
                        out += '<a title="Edit Service" class="text-info fs-14 lh-1" href="{{url("admincp/editservice")}}/' + id + '" ><i class="ri-edit-line"></i></a>&nbsp;&nbsp;&nbsp;';
                        out += '<a title="Delete Service" class="text-danger fs-14 lh-1" onClick="deleteService(' + id + ');"><i class="ri-delete-bin-5-line"></i></a>';
                        return out;
                    }
                },
            ]
    ...
    function deleteService(id) {
        $.ajax({
            url: admin_path + 'servicedeletemodal/' + id,
    ...
    function faqStatusChange(id) {
        $.ajax({
            url: admin_path + 'changeservicestatusmodal/' + id,
            success: function(response) {
                if (response.status == 'success') {
