...
class Faq extends Model implements Auditable
{
    ...
    public static function getFaq($table_name, $datatable_fields, $conditions_array, $getfiled, $request, $join_str = array()) {
        DB::enableQueryLog();
        $output = array();
        ...
        if (!$data_d->isEmpty()) {
            foreach ($data_d as $key => $d) {
                $data_d[$key]->answer = strip_tags($d->answer);
                $data_d[$key]->created_at = Common::adminconvertTimezone($d->created_at, 'm-d-Y h:i A');
                $data_d[$key]->updated_at = Common::adminconvertTimezone($d->updated_at, 'm-d-Y h:i A');
            }
        }
        $output['data'] = $data_d;
